...
def test_model():
    ...
    datarootA = opt.datarootA
    datarootB = opt.datarootB
    ...
    for root, _, fnames in sorted(os.walk(datarootA)):
        for fname in fnames:
            path = os.path.join(root, fname)
            mem_A_np.append(np.asarray(Image.open(path).convert('RGB').resize((64,64), Image.BICUBIC)))

    for root, _, fnames in sorted(os.walk(datarootB)):
        for fname in fnames:
            path = os.path.join(root, fname)
            mem_B_np.append(np.asarray(Image.open(path).convert('RGB').resize((64,64), Image.BICUBIC)))

    

    arrA = np.stack(mem_A_np)
    arrB = np.stack(mem_B_np)

    arrA = np.transpose(arrA, (0,3,1,2))
    arrA = arrA / 127.5 - 1.
    devA = arrA.astype('float32')

    arrB = np.transpose(arrB, (0,3,1,2))
    arrB = arrB / 127.5 - 1.
    devB = arrB.astype('float32')

    batch_size = min(10, len(mem_A_np))
    dev_dataset = AlignedIterator(devA, devB, batch_size=batch_size)
    ...
    if opt.metric == 'bpp':
        ...
        for i, vis_data in enumerate(dev_dataset):
            with torch.no_grad():
                ...
                prior_z_B = Variable(real_A.data.new(batch_size, opt.nlatent, 1, 1).normal_(0, 1))
            ...
            real_A, rec_A,real_B, rec_B = Variable(visuals['real_A']), Variable(visuals['rec_A']),Variable(visuals['real_B']),Variable(visuals['rec_B'])
            if use_gpu:
                real_A = real_A.cuda()
                rec_A = rec_A.cuda()
                real_B = real_B.cuda()
            ...
            l1_lossA.append(F.l1_loss(real_A, rec_A).data.cpu())
            l2_lossA.append(F.mse_loss(real_A, rec_A).data.cpu())
            l1_lossB.append(F.l1_loss(real_B, rec_B).data.cpu())
            l2_lossB.append(F.mse_loss(real_B, rec_B).data.cpu())
        ...
        print("L1_A: %.4f" % (np.mean(l1_lossA)))
        print("L2_A: %.4f" % (np.mean(l2_lossA)))
        print("L1_B: %.4f" % (np.mean(l1_lossB)))
        print("L2_B: %.4f" % (np.mean(l2_lossB)))
